...
@section('content')
    <div class="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
        ...
        <form action="{{ route('mesures.update', $mesure->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                <label for="min" class="block text-gray-600 font-semibold">Valeur minimale</label>
                <input type="number" step="0.01" name="min" id="min" value="{{ old('min', $mesure->min) }}"
                    required class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400">
                @error('min')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label for="max" class="block text-gray-600 font-semibold">Valeur maximale</label>
                <input type="number" step="0.01" name="max" id="max" value="{{ old('max', $mesure->max) }}"
                    required class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400">
                @error('max')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
