...
<div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-lg mt-8">
    ...
    <div class="bg-gray-100 p-6 rounded-lg shadow-sm">
        <p class="text-lg"><strong class="font-semibold">📦 Commande #{{ $detail->commande->id }}</strong></p>
        <p class="text-lg"><strong class="font-semibold">👗 Modèle :</strong> {{ $detail->modele->nom }}</p>
        <p class="text-lg"><strong class="font-semibold">Quantité :</strong> x{{ $detail->quantite }}</p>
        <p class="text-lg"><strong class="font-semibold">Prix unitaire :</strong>
            <span class="text-green-600 font-semibold">{{ number_format($detail->prix_unitaire, 2) }} €</span>
        </p>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm mt-4">
        <form action="{{ route('commandes.assigner_couturiere', $detail->id) }}" method="POST">
            @csrf
            <div class="flex items-center space-x-4">
                <select name="user_id" class="form-select border-gray-300 rounded-lg p-2 text-gray-700 shadow-sm focus:ring focus:ring-blue-300 w-full">
                    ...
                    @foreach($couturieres as $couturiere)
                        <option value="{{ $couturiere->id }}" {{ $detail->user_id == $couturiere->id ? 'selected' : '' }}>
                            {{ $couturiere->name }}
                        </option>
                    @endforeach
                </select>
    ...
    <div class="mt-8">
        <a href="{{ route('commandes.show', $detail->commande->id) }}"
