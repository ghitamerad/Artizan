...
@section('content')
    <div class="max-w-2xl mx-auto p-6 space-y-6">
        <h2 class="text-2xl font-bold text-gray-800">Modifier l’attribut : {{ $valeur->nom }}</h2>
        ...
        <form action="{{ route('valeurs.update', $valeur) }}" method="POST" enctype="multipart/form-data"
            ...
            {{-- Type d’attribut lié --}}
            <div>
                <label class="block text-sm font-medium text-gray-700">Type d’attribut</label>
                <input type="text" value="{{ $valeur->attribut->nom }}" readonly
                    class="mt-1 block w-full rounded-xl bg-gray-100 border border-gray-300 text-gray-700 px-4 py-2 shadow-sm cursor-not-allowed">
            </div>
            ...
            <div>
                ...
                <input type="text" name="nom" id="nom" value="{{ old('nom', $valeur->nom) }}"
                    class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-base py-2 px-4"
                    required>
                @error('nom')
            ...
            @if ($valeur->image)
                <div>
                    ...
                    <div class="mt-2">
                        <img src="{{ asset('storage/' . $valeur->image) }}" alt="Image de l’attribut"
                            class="w-40 h-auto rounded shadow border border-gray-200">
                    </div>
                </div>
            @endif
            ...
            <label for="custom" class="text-sm text-gray-700">Attribut personnalisable</label>
        </div> --}}
