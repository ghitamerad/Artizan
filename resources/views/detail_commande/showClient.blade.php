...
@section('content')
    <div class="container mx-auto p-6 mt-10">
        <h2 class="text-2xl font-bold text-gray-800 mb-4">📄 Détails de la commande #{{ $commande->id }}</h2>

        <div class="bg-white p-6 rounded-lg shadow-md mb-6">
            <p class="text-gray-600">Date : <span class="font-semibold">{{ $commande->created_at->format('d/m/Y') }}</span>
            </p>
            ...
            <p class="text-gray-600">Montant total : <span
                    class="font-semibold">{{ number_format($commande->montant_total, 2) }} €</span></p>
        </div>
            ...
            @foreach ($detailsCommande as $detail)
                <div class="p-4 border rounded-lg mb-3">
                    ...
                    @php
                        ...
                        } elseif ($statut === 'Null' || $statut === 'validee') {
                            $color = 'text-gray-500';
                        }
                    @endphp

                    <p class="text-sm text-gray-500 {{ $color }}">statut : {{ $statut ?? 'en cours' }}</p>
        ...
        <a href="{{ route('detail-commandes.index') }}"
            class="mt-4 inline-block px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition">
